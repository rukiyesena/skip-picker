...
const SkipListGrid = ({ skip }) => {
  ...
  return (
    <React.Fragment>
      <Card onButtonClick={() => { skip.allows_heavy_waste ? onButtonClick() : '' }} selected={skip.selected}>
        <div className={`basic-skip theme-skip`}>
          <div className="img-wrapper">
            ...
            <ul className="trending-label" style={{ paddingLeft: "0", marginBottom: "0" }}>
              {skip.allowed_on_road == false ? <li className="allow-road" style={{ display: "ruby" }}>
                <FaInfoCircle />  Not Allowed On The Road</li> : null}
              {!skip.allows_heavy_waste ? <li className="heavy-waste" style={{ display: "ruby" }}><FaInfoCircle />  Not Suitable for Heavy Waste</li> : null}
          ...
          <div className="product-detail">
            ...
            <div className="addtocart_btn" style={{ width: "auto" }}>
              <button
                className={`bg-primary text-white px-4 py-2 rounded ${skip.selected ? 'bg-success' : ''} ${!skip.allows_heavy_waste ? 'opacity-50' : ''}`}
